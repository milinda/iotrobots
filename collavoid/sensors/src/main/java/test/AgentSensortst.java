...
public class AgentSensortst extends AbstractSensor {

    private static Logger LOG = LoggerFactory.getLogger(AgentSensortst.class);

    public static void main(String[] args) {
        Map<String, String> properties = getProperties();
        String jarName = new File(AgentSensortst.class.getProtectionDomain()
        ...
        for (int i = 0; i < no; i++) {
            ...
            SensorSubmitter.submitSensor(properties, jarName,
                    AgentSensortst.class.getCanonicalName(), Arrays.asList(s));
        }
    }
    ...
    public void open(final SensorContext sensorContext) {
        AgentControllerIotTest controllerIot;

        LOG.info("-----------------------test------------------------start");
        controllerIot = new AgentControllerIotTest(sensorContext);
                ...
                new MessageReceiver() {
                    ...
                    public void onMessage(Object message) {
                        if (message instanceof MessageContext) {
                            ...
                            Map<String, Object> props = ((MessageContext) message).getProperties();
//                            long index=Long.parseLong(props.get("seq").toString());
                            Twist_ vel = (Twist_) Methods_RMQ.deSerialize(kryo, body, Twist_.class);
                            velqueue.offer(vel);
                            if (sensorContext.getProperty(Constant_storm.IotCloud.AGENT_INDEX).equals("0"))
                                LOG.info("Command received " + index++);
    ...
    private class AgentSensorConfigurator extends AbstractConfigurator {
        ...
        public SensorContext configure(SiteContext siteContext, Map conf) {
            ...
            BlockingQueue<Object> odombytequeue = new LinkedBlockingDeque<>();
            context.addProperty(Constant_storm.IotCloud.ODOM_BYTE_QUEUE, odombytequeue);
...
//            Map Props = new HashMap();
//            Props.put("exchange", msgContexts.get(Constant_storm.Components.ODOMETRY_COMPONENT).EXCHANGE_NAME);
//            Props.put("routingKey", msgContexts.get(Constant_storm.Components.ODOMETRY_COMPONENT).ROUTING_KEY);
//            Props.put("queueName", msgContexts.get(Constant_storm.Components.ODOMETRY_COMPONENT).QUEUE_NAME);
//            Channel channel = createChannel("odom_in", Props, Direction.IN, 1024);
//            channel.setGrouped(true);
//            context.addChannel(Constant_storm.IotCloud.TRANSPORT, channel);
