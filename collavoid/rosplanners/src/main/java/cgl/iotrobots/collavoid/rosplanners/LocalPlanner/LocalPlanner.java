...
import cgl.iotrobots.collavoid.rosplanners.ROSAgent.ROSAgent;
import cgl.iotrobots.collavoid.rosplanners.utils.Angles;
import cgl.iotrobots.collavoid.rosplanners.utils.Parameters;
import cgl.iotrobots.collavoid.rosplanners.utils.Vector2;
import cgl.iotrobots.collavoid.rosplanners.utils.utils;
...
//        me.msgPublisher.publishPlan( local_plan,id, l_plan_pub_);
    ...
    boolean stopWithAccLimits(PoseStamped global_pose, final Twist robot_vel, Twist cmd_vel) {
        ...
        double vx = utils.sign(robot_vel.getLinear().getX()) * Math.max(0.0, (Math.abs(robot_vel.getLinear().getX()) - me.acc_lim_x_ * me.simPeriod));
        double vy = utils.sign(robot_vel.getLinear().getY()) * Math.max(0.0, (Math.abs(robot_vel.getLinear().getY()) - me.acc_lim_y_ * me.simPeriod));

        double vth = utils.sign(robot_vel.getAngular().getZ()) * Math.max(0.0, (Math.abs(robot_vel.getAngular().getZ()) - me.acc_lim_th_ * me.simPeriod));
    ...
    boolean rotateToGoal(final PoseStamped global_pose, final Twist robot_vel, double goal_th, Twist cmd_vel) {
        ...
        v_th_samp = utils.sign(v_th_samp) * Math.min(Math.max(Math.abs(v_th_samp), min_acc_vel), max_acc_vel);

        //we also want to make sure to send a velocity that allows us to stop when we reach the goal given our acceleration limits
        double max_speed_to_stop = Math.sqrt(2 * me.acc_lim_th_ * Math.abs(ang_diff));//how to get this???

        v_th_samp = utils.sign(v_th_samp) * Math.min(max_speed_to_stop, Math.abs(v_th_samp));
        if (Math.abs(v_th_samp) <= 0.0 * me.min_vel_th_inplace_)//???????????
            v_th_samp = 0.0;
        else if (Math.abs(v_th_samp) < me.min_vel_th_inplace_)
            v_th_samp = utils.sign(v_th_samp) * Math.max(me.min_vel_th_inplace_, Math.abs(v_th_samp));
